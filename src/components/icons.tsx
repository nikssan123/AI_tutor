...
/**
 * The Google "G" — the one icon here that breaks every house rule above, on
 * purpose.
 *
 * It does not go through `Svg`, because that wrapper forces `currentColor` and
 * a 1.5 stroke, and this mark is neither: it is four fixed brand colours on a
 * 48×48 grid, filled rather than stroked. Google's terms allow the mark to be
 * scaled and placed on our own surface, but not recoloured, so the hex values
 * are literal — a token here would be a promise we are not allowed to keep.
 *
 * The four colours are legible on both themes (Google ships the same mark on
 * its own `#131314` dark button), so there is no per-theme variant.
 *
 * Decorative like the rest: the button beside it always says "Google" in words,
 * which is also what keeps the meaning for anyone who cannot see the colours.
 */
