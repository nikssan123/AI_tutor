...
authored-page substrate, §12.2's quality gate and eight `/guides` pages are
in — so from here the remaining work is acquisition and money rather than
...
> **The template and eight of these are built** — every one that can be answered
> honestly: `why-do-i-forget-what-i-learn` · `why-am-i-stuck-in-tutorial-hell` ·
> `what-should-i-learn-after-python-basics` ·
> `how-do-i-know-if-im-actually-improving` · `how-long-does-it-take-to-learn-sql` ·
> `how-long-does-it-take-to-learn-python` ·
> `how-many-hours-a-week-to-learn-a-new-skill` ·
> `best-way-to-learn-a-skill-as-an-adult`. Three things this list did not
> anticipate:
>
...
>   thing §12 exists to stop, and they stay unwritten until there is a pack and
>   a cohort to answer them from.
...
| **Faceted nav** | **`noindex` on every faceted/filtered/parameterised URL.** Canonical to the bare view. Non-negotiable — this is the #1 index-bloat source. **The other one is the trailing slash, and it cost us a feature:** a PostHog reverse proxy at `/ingest/*` needs `skipTrailingSlashRedirect: true`, which turns off the 308 that enforces `trailingSlash: false` — so `/learn/{topic}/` would begin answering 200 beside `/learn/{topic}`, a duplicate pair on all 93 indexable URLs at once. Canonicals would mostly cover it, and "mostly" is the wrong bar for a crawl-budget invariant on a domain with no history. Declined deliberately; if ad-block resilience is ever wanted, it belongs in a Cloudflare Worker at the edge, where Next's redirect behaviour is untouched. |
| **Site search** | `/search` is `noindex,follow`. |
| **Crawl budget** | Sitemap contains only indexable pages; faceted URLs `noindex`; app routes disallowed; 301 chains eliminated; IndexNow ping to Bing on publish. **Built** (`src/lib/seo/indexnow.ts`, `/indexnow/{key}.txt`, `pnpm indexnow`). Two things worth stating plainly: **Google has never joined IndexNow**, so this speeds up Bing, Yandex, Seznam and Naver and does nothing for the engine §9 is aimed at — worth having on a domain with no crawl history, not worth mistaking for a Google lever. And **publish here is a deploy**: the ranking pages come from files, so there is no per-page publish event to hook and the sitemap is the source, which also guarantees the submission and the robots tags cannot disagree. |
...
| **E10** SEO infrastructure | ✅ Done | `sitemap.ts`, `robots.ts`, `src/lib/seo/` — metadata, JSON-LD, the share cards, **the internal-link renderer and the quality-score job** (`src/lib/guides/{links,quality}.ts`), and **IndexNow** (`indexnow.ts`, `/indexnow/{key}.txt`, `pnpm indexnow`), which was §13.3's last unbuilt row. Lighthouse and GSC/Bing verification wait on a deployed origin |
...
passed first time, which proves nothing. Six defects across the next six did
not, and all six were invisible on the rendered page: a source nothing cited, a
guide leaning on catalogue-wide figures and so earning no inbound link from any
subject, three sources across two domains, and a `{{…}}` in a link anchor that
rendered as braces. Every one was fixed by writing more specifically. Nothing
was relaxed.

**And a subject page was quietly showing seven guide cards**, which is a
directory rather than a section (§8.5.1). The cap is four — placed in
`guidesForSubject` rather than in the page, because the inbound count is
computed from the same list, and a cap applied only at render would leave the
gate counting links the page does not draw. Ordering is by how often a guide
quotes that subject's own figures: non-arbitrary, and nobody has to author it.
...
### E12 — Content production (days 24–28) — 🟡 8 pages of 50
